...
public class RedisProvider extends RedisCache implements IRedisProvider{
	public static final FLLogger log = FLLogger.getLogger("redis/log");

	public RedisProvider() {

	}
	public RedisProvider(Map config) {
		CachePool._config = config;
	}
	/**
...
//			// return null;
//			// }
//			String script="return redis.call('HGETALL', '"+key+"')";//redis.call('HGETALL', key)
...
//					try {
					try (Jedis jedis = CachePool.getInstance().getJedis()) {
						...
						if (Lib.timeDiffSecond(Lib.getCurrentDate(), time_start) > Constants.REDIS_TIME_TIMEOUT_LIMIT) {
...
//			Map<String, String> myList =  hGetAllByLuaScript(key);
	...
	public <T> List<T> getListByIndex(Class<T> entityClass, String indexFieldName, String value, int startIndex,
		...
		try (Jedis jedis = CachePool.getInstance().getJedis()) {
			...
			return resultList.stream().distinct().collect(Collectors.toList());//resultList;
		} catch (Exception ex) {
	...
	public <T> boolean delete(Class<T> entityClass, String keyvalue) {
		...
		try (Jedis jedis = CachePool.getInstance().getJedis()) {
			...
			if (existedObj == null || Lib.isBlank(primaryKeyvalue))
				return false;
			...
			try {
				...
				if (indexes != null && indexes.length > 0)
					for (String idx : indexes)
						trans.lrem(getIndexKey(existedObj, idx), 0, keyvalue);
				trans.exec();
			} catch (Exception ex) {
				trans.discard();
				log.error("delete", ex);
