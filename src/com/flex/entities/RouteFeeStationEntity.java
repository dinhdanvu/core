...
public class RouteFeeStationEntity extends BaseEntity{
	private String db;
	private int id;
	 ...
	 */
	public void validateRouteFeeStation(){
		...
		//fee_station_name không quá 255 kí tự
		if(!Lib.isBlank(this.fee_station_name)&& this.fee_station_name.length()>255){
			String name1 = this.fee_station_name.substring(0, Math.min(this.fee_station_name.length(), 255));
			this.setFee_station_name(name1);
		}
			
		// address không quá 1000 kí tự
		if(!Lib.isBlank(this.address1)&& this.address1.length()>1000){
			String name1 = this.address1.substring(0, Math.min(this.address1.length(), 1000));
			this.setAddress1(name1);
		}
		
		// address không quá 1000 kí tự
		if(!Lib.isBlank(this.address2)&& this.address2.length()>1000){
			String name1 = this.address2.substring(0, Math.min(this.address2.length(), 1000));
			this.setAddress2(name1);
		}
